...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val allQuestions = postings.filter(p => p.postingType == 1).map(q => (q.id, q))
    val allAnswers = postings.filter(p => p.postingType == 2).map(a => (a.parentId.getOrElse(-1), a))
    val joinedByQId = allQuestions.join(allAnswers)
    val groupedByQId = joinedByQId.groupByKey
    groupedByQId
  }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    val highScoresByQId: RDD[(QID, HighScore)] = grouped.map { case (k, v) => (k, answerHighScore(v.map { case (_, a) => a }.toArray)) }
    val questionsById: RDD[(QID, Question)] = grouped.map { case (qid, iter) => (qid, iter.head._1) }
    questionsById.join(highScoresByQId).values
  }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val stat = vs.groupBy(_._1).map { case (k, v) => (k, v.size) }.toSeq.sortBy(_._2)
      val sumStat = stat.foldLeft(0)(_ + _._2)

      val langLabel: String = langs(stat.last._1 / langSpread)
      // most common language in the cluster
      val langPercent: Double = stat.last._2 / sumStat * 100
